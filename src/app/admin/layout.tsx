...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!isAdmin) return (
    <div className="min-h-screen flex items-center justify-center bg-surface-50">
      <div className="card p-8 max-w-sm w-full text-center">
        ...
        <p className="text-slate-500 text-sm mb-2">{user.email} is not an admin.</p>
        <p className="font-mono text-xs text-slate-400 mb-4">
          Add this email to NEXT_PUBLIC_ADMIN_EMAILS in .env.local and restart.
        </p>
  ...
  // Signed in and is admin — show CMS
  return (
      ...
      <aside className="w-56 flex-shrink-0 bg-white border-r border-surface-200 flex flex-col">
        ...
        <div className="p-3 border-t border-surface-100 space-y-0.5">
          ...
          <button onClick={logout}
            className="admin-nav-link w-full text-left text-red-500 hover:text-red-600 hover:bg-red-50">
